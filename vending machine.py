...
db= mysql.connector .connect(user="local",host="root",password="kv@123")
c=db.cursor()
c.execute("Create Database VM;db= mysql.connector .connect(user="local",host="root",password="kv@123")
c=db.cursor()
c.execute("Create Database VM")
c.execute("Use VM")
c.execute("Create table Stock(Name_of_the_item varchar(50) not null,price int(5) not null,code int(5) Primary Key,Quanity int(5) not null)")")
c.execute("Use VM;")
c.execute("Create table Stock(Name_of_the_item varchar(50) not null,price int(5) not null,code int(5) Primary Key,Quanity int(5) not null);")
class Coin(object):
...
def StockItem(Vm):
    ...
    Vm.addItem(itm)
    c.execute("Insert into stock values(name,price,code,quant);")
